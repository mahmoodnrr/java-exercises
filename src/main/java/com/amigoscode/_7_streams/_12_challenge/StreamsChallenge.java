...
public class StreamsChallenge {
    ...
    public static void main(String[] args) {
        ...
        Map<String, Double> sum = transactions.stream()
                .collect(
                        Collectors.groupingBy(
                                Transaction::type, Collectors.summingDouble(Transaction::amount))
                );
        System.out.println();
        ...
        System.out.println();
        Map<String, Double> average = transactions.stream()
                .collect(
                        Collectors.groupingBy(
                                Transaction::type, Collectors.averagingDouble(Transaction::amount)));
        ...
        transactions.stream()
                .collect(
                        Collectors.groupingBy(
                                t -> t.date().getMonth(), Collectors.counting()))
        ...
        DoubleSummaryStatistics summary = transactions.stream()
                ...
                .summaryStatistics();
        System.out.println("Total transactions: " + summary.getCount() +
                " | Total amount: " + summary.getSum() +
                " | Avg amount: " + summary.getAverage());
...
//        System.out.println("Total transactions: " + totalCount + " | Total amount: $" + totalAmount + " | Avg amount: $" + avgAmount);
        ...
        //           Print each duplicate amount and how many times it appearst
        transactions.stream()
        ...
        List<Transaction> sort = transactions.stream()
                .sorted(Comparator.comparing(Transaction::date)
                        .thenComparing(Transaction::amount).reversed())
