...
class BDDMockitoTest {
    ...
    void canThrowException() {
        //Given
        Order order = new Order("ORD-1", "CUST-1", 99.99);
        given(paymentService.charge("CUST-1", 99.99))
                .willThrow(new RuntimeException("Payment gateway down"));
    ...
    @Test
    void canUseDifferentTestStyles() {
        Order order = new Order("ORD-1", "CUST-1", 99.99);
        ...
        // then
        assertThrows(RuntimeException.class, () -> orderService.placeOrder(order));
        then(paymentService).should(times(1)).charge("CUST-1", 99.99);
        ...
        // Verify
        assertThat(order1.getStatus()).isEqualTo("COMPLETED");

        verify(paymentService, times(2)).charge(anyString(), anyDouble());
